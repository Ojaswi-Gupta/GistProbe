...
def perform_clustering(df):
    ...
    # Package word scores for word cloud generation
    scores = X.sum(axis=0).A1
    word_scores = {terms[i]: float(scores[i]) for i in range(len(terms)) if scores[i] > 0}

    return df, counts, takeaways, metrics, word_scores